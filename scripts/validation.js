...
function inputValidator({ selector, errorSelector, inputErrorClass, validators, onInput }) {
  ...
  function validate() {
    return Array.from(validators).every((item) => item.rule(input.value, item.param) === true);
  }
...
function enableValidation({
  ...
  if (form === null || button === null) {
    console.error('Попап не содержит нужных DOM элементов для работы валидации');
    return;
  }
  ...
  // ивенты для обновления текста в кнопке во время ожидания ответа от сервера
